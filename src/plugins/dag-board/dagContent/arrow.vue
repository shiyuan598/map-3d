  ...
  methods: {
    ...
    computedLink() {
      ...
      if (!this.DataAll) {
        ...
        if (this.isVertical()) {
          ...
          t_X = t_Pos.pos_x;
          t_Y = t_Pos.pos_y + 15;
          return `M ${f_X} ${f_Y}  Q ${f_X + 30} ${f_Y} ${(t_X + f_X) /
            2} ${(t_Y + f_Y) / 2} T ${t_X} ${t_Y}`;
        }
      }
    },
    ...
    computedArrow() {
      ...
      if (!this.DataAll) {
        ...
        const { dst_input_idx, dst_node_id } = this.each;
        const t_Pos = this.DataAll.nodes.find(item => item.id === dst_node_id);
        let t_X =
          t_Pos.pos_x +
          (180 / (t_Pos.in_ports.length + 1)) * (dst_input_idx + 1);
        let t_Y = t_Pos.pos_y;
        if (this.isVertical()) {
          return `${t_X} ${t_Y + 6} ${t_X - 6} ${t_Y - 6} ${t_X + 6} ${t_Y -
            6}`;
        } else {
          let t_X = t_Pos.pos_x;
          let t_Y = t_Pos.pos_y + 15;
          return `${t_X - 5} ${t_Y + 5} ${t_X - 5} ${t_Y - 5} ${t_X + 5} ${t_Y -
            3}`;
        }
      }
    },
